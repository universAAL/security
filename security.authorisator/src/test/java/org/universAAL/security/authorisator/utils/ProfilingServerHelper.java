...
public class ProfilingServerHelper {
	...
	private static final String OUTPUT_GETSUBPROFILES = NAMESPACE + "outputSubprofile";
	...
	public static void logUsers(User[] users) {
		...
		LogUtils.logDebug(mc, ProfilingServerHelper.class, "logUsers", new Object[] { s }, null);
	}
	...
	public String addSubprofileToUser(User profilable, SubProfile subProfile) {
		...
		LogUtils.logDebug(mc, this.getClass(), "addSubprofileToUser",
				new Object[] { "ui subprofile: " + subProfile.getURI() + " created and connection attempt to user: "
						+ profilable.getURI() + " returned status: " + resp.getCallStatus().name() },
				null);
	...
	public String changeSubProfile(SubProfile subProfile) {
		ServiceCaller caller = new DefaultServiceCaller(mc);
		ServiceRequest req = new ServiceRequest(new ProfilingService(), null);
		req.addChangeEffect(new String[] { ProfilingService.PROP_CONTROLS, Profilable.PROP_HAS_PROFILE,
				Profile.PROP_HAS_SUB_PROFILE }, subProfile);
		ServiceResponse resp = caller.call(req);

		LogUtils.logDebug(mc, this.getClass(), "changeSubProfile", new Object[] { "ui subprofile: "
				+ subProfile.getURI() + " change reguest returned status: " + resp.getCallStatus().name() }, null);

		return resp.getCallStatus().name();
	}

	/**
	 * Retrieves {@link SecuritySubprofile} that belongs to a given {@link User}
		...
		if (resp.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				if (subProfiles == null || subProfiles.size() == 0) {
					LogUtils.logInfo(mc, this.getClass(), "getSecuritySubprofilesForUser",
				...
				while (iter.hasNext()) {
					...
					if (subProfile.getClassURI().equals(SecuritySubprofile.MY_URI)) {
						LogUtils.logInfo(mc, this.getClass(), "getSecuritySubprofilesForUser", new Object[] {
								"Following SecuritySubprofile obtained from Profiling server: " + subProfile.getURI() },
								null);
				...
				LogUtils.logError(mc, this.getClass(), "getSecuritySubprofileForUser", new Object[] { "exception: " },
						e);
				return null;
			}
		} else {
			LogUtils.logWarn(mc, this.getClass(), "getSecuritySubprofileForUser",
		...
		if (resp.getCallStatus() == CallStatus.succeeded) {
			LogUtils.logDebug(mc, this.getClass(),
					"addSubprofileToUserProfile", new Object[] {
							"SubProfile: " + subProfile.getURI() + " added to UserProfile: ", userProfile.getURI() },
					null);
			return true;
		} else {
			LogUtils.logDebug(mc, this.getClass(), "addSubprofileToUserProfile",
					new Object[] { "callstatus : " + resp.getCallStatus().name() }, null);
			return false;
		}
	}
