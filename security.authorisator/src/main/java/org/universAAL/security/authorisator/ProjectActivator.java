...
public class ProjectActivator implements ModuleActivator {
	...
	public void start(ModuleContext ctxt) throws Exception {
		...
		addRoleSP.addFilteringInput(SUBPROFILE, SecuritySubprofile.MY_URI, 1, 1,
				SecuritySubProfileRoleManagement.pp_subprofile);
		...
		remRoleSP.addFilteringInput(SUBPROFILE, SecuritySubprofile.MY_URI, 1, 1,
				SecuritySubProfileRoleManagement.pp_subprofile);
		...
		addRole.addInputWithAddEffect(SUBROLE, Role.MY_URI, 1, 1,
		...
		remRole.addInputWithRemoveEffect(SUBROLE, Role.MY_URI, 1, 1,
				new String[] { RoleManagementService.PROP_ROLE, Role.PROP_SUB_ROLES });
		...
		addAR.addInputWithAddEffect(ACCESS_RIGHT, AccessRight.MY_URI, 1, 1,
		...
		remAR.addInputWithRemoveEffect(ACCESS_RIGHT, AccessRight.MY_URI, 1, 1,
				new String[] { RoleManagementService.PROP_ROLE, Role.PROP_HAS_ACCESS_RIGHTS });
		profs[5] = remAR.getProfile();

		// change AccessRight
		RoleMngServiceProfile changeAR = new RoleMngServiceProfile(CHANGE_AR);
		changeAR.addInputWithChangeEffect(ACCESS_RIGHT, AccessRight.MY_URI, 1, 1,
				new String[] { RoleManagementService.PROP_ROLE, Role.PROP_HAS_ACCESS_RIGHTS });
		profs[6] = changeAR.getProfile();

		// get all Access Rights
		RoleMngServiceProfile getAR = new RoleMngServiceProfile(GET_AR);
		getAR.addOutput(ACCESS_RIGHT, AccessRight.MY_URI, 0, -1,
				new String[] { RoleManagementService.PROP_ROLE, Role.PROP_HAS_ACCESS_RIGHTS });
		profs[13] = getAR.getProfile();

		// TODO all the previous service should check authorisation for the user
		...
		readCheckCU.addFilteringInput(USER, User.MY_URI, 0, 1,
				new String[] { AuthorizationService.PROP_CHALLENGER_USER });
		readCheckCU.addFilteringInput(ASSET, Asset.MY_URI, 1, 1,
		...
		changeCheckCU.addFilteringInput(USER, User.MY_URI, 0, 1,
				new String[] { AuthorizationService.PROP_CHALLENGER_USER });
		changeCheckCU.addInputWithChangeEffect(ASSET, Asset.MY_URI, 1, 1,
				new String[] { AuthorizationService.PROP_ASSET_ACCESS });
		...
		addCheckCU.addFilteringInput(USER, User.MY_URI, 0, 1,
				new String[] { AuthorizationService.PROP_CHALLENGER_USER });
		addCheckCU.addInputWithAddEffect(ASSET, Asset.MY_URI, 1, 1,
				new String[] { AuthorizationService.PROP_ASSET_ACCESS });
		...
		remCheckCU.addFilteringInput(USER, User.MY_URI, 0, 1,
				new String[] { AuthorizationService.PROP_CHALLENGER_USER });
		remCheckCU.addInputWithRemoveEffect(ASSET, Asset.MY_URI, 1, 1,
				new String[] { AuthorizationService.PROP_ASSET_ACCESS });
		profs[10] = remCheckCU.getProfile();

		callee = new AuthorisatorCallee(context, profs);

		// TODO: create a context publisher which publishes user access to
		// assets for accountability (combined with CHe)
