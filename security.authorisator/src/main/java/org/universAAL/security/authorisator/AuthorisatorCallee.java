...
public class AuthorisatorCallee extends ServiceCallee {
	 ...
	 */
	public AuthorisatorCallee(ModuleContext context, ServiceProfile[] realizedServices) {
	...
	public ServiceResponse handleCall(ServiceCall call) {
		...
		if (callURI.contains("check")) {
			...
			LogUtils.logDebug(owner, getClass(), "handleCall", "Checking user AccesRights: "
	...
	private boolean setProperty(Resource r, String prop) {
		...
		if (propvalue == null) {
			...
			String resp = query.unserialisedQuery(CHeQuerrier.getQuery(CHeQuerrier.getResource("removeProperty.sparql"),
	...
	private boolean updateObject(Resource r) {
		...
		String prefixes = split[0];
		String serialValue = split[1];
		String resp = query.unserialisedQuery(CHeQuerrier.getQuery(CHeQuerrier.getResource("updateFullObject.sparql"),
	...
	private Object getAllObjectsOfType(String classuri) {
		...
		Object o = query.query(CHeQuerrier.getQuery(CHeQuerrier.getResource("getObjectType.sparql"),
				new String[] { AUX_BAG_OBJECT, AUX_BAG_PROP, classuri }));
