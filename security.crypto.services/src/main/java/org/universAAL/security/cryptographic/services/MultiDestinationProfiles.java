...
public class MultiDestinationProfiles extends EncryptionService {
	...
	static void initialize(ModuleContext mc) {
		...
		add.addFilteringInput(PARAM_KEY_RING, KeyRing.MY_URI, 1, 1, new String [] {EncryptionService.PROP_ENCRYPTION,AsymmetricEncryption.PROP_KEY_RING});
		...
		rem.addFilteringInput(PARAM_KEY_RING, KeyRing.MY_URI, 1, 1, new String [] {EncryptionService.PROP_ENCRYPTION,AsymmetricEncryption.PROP_KEY_RING});
		//Effect is to remove a DestinataryEncryptionSessionKey
		rem.addInputWithRemoveEffect(PARAM_DESTINATION, DestinataryEncryptedSessionKey.MY_URI, 1, -1, new String [] {EncryptionService.PROP_ENCRYPTED_RESOURCE, MultidestinationEncryptedResource.PROP_DESTINATARIES});
		rem.addOutput(PARAM_ENCRYPTED_RESOURCE, MultidestinationEncryptedResource.MY_URI, 1, 1, new String [] {EncryptionService.PROP_ENCRYPTED_RESOURCE});
