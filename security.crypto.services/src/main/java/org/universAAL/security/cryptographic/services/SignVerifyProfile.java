...
static private final String NAMESPACE = "http://security.universAAL.org/JavaDigitalSignatureServices#";
	...
	static void initialize(ModuleContext mc) {
		...
		sign.addOutput(SIGNED_RESOURCE, SignedResource.MY_URI, 1, 1, new String[]{PROP_SIGN});
		...
		//TODO expand profile and serviceCallee for multiple signature procedure
		...
		emVerify.addFilteringInput(DIG_METHOD, Digest.MY_URI, 1, 1, new String[]{PROP_SIGNED_RESOURCE, SignedResource.PROP_DIGEST});
			// Asymmetric has to have the keyring
		...
		emVerify.addInstanceLevelRestriction(
				MergedRestriction.getAllValuesRestrictionWithCardinality(KeyRing.PROP_PUBLIC_KEY, TypeMapper.getDatatypeURI(Base64Binary.class), 1, 1),
				new String[]{PROP_SIGNED_RESOURCE,SignedResource.PROP_ASYMMETRIC,AsymmetricEncryption.PROP_KEY_RING,KeyRing.PROP_PUBLIC_KEY});
		...
		exVerify.addFilteringInput(DIG_METHOD, Digest.MY_URI, 1, 1, new String[]{PROP_SIGNED_RESOURCE, SignedResource.PROP_DIGEST});
			// Asymmetric has to have the keyring
