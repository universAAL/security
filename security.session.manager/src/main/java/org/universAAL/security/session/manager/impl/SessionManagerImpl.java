...
public class SessionManagerImpl implements SessionManager {
	...
	private static Session INVALID = (Session) Session.getInstance(Session.MY_URI, Session.INSTANCE_INVALID_SESSION);
	...
	/**
	 * The main map for sessions, mapping user URI to their current Session.
	 */
	...
	public void userAuthenticatedTo(User usr, Device dvc) {
		...
		if (canLocationBoundedSessionBeIssued(usr, dvc)) {
			/*
			 * if there is a location then, uplift to the location where there
			 * are no other users.
			 */

			s = new LocationBoundSession(constructSessionURI(usr));
			s.setProperty(LocationBoundSession.PROP_BOUNDED_LOCATION, locationRoot.getMaxUser(usr).getLocation());
		} else {
			/*
			 * if there is not a location of the authentication create a device
			 * bounded session for the most parent device.
			 */
	...
	public void userDeauthenticatedFrom(User usr, Device dvc) {
		// TODO Discussion: user loggin of in one device means it logs of in the
		// space?
	...
	public void userLocationChange(User usr, Location loc) {
		...
		// check if DeviceBoundSession can be uplifted to a
	...
	public Set<User> validUsersForLocation(Location loc) {
		...
		// add the users with DeviceBoundSessions in the devices in this
		// location.
